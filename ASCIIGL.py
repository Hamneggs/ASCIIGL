"""
	...
	---FULLSCREEN MODE---
	ASCIIGL Surfaces can be run in fullscreen. Fullscreen mode simply means
	that the rendering surface takes up the entire console window. 
	This means that size change checks must be performed, either by ASCIIGL,
	or by code external to the library.
	ASCIIGL has two settings to perform the size check. Either only at the creation
	of the rendering surface, or at a specifiable interval of frames located in
	ASCIIGLConstants.
	Alternatively, your program can call the check directly if you know when the
	terminal will change size.
	SIZE CHECK CURRENTLY ONLY WORKS ON *NIX.
	
	---SHADERS---
	ASCIIGL has shader support through the use of functions as variables.. A shader is defined as
	a function with 5 parameters--x, y, XframeSize, YframeSize, and char---that ASCIIGL will pass into your
	shader. It MUST FUCKING RETURN A STRING THAT TAKES THE SPACE OF A SINGLE CHARACTER
	WHEN DISPLAYED.
...
class ASCIIGL(object):
	...
	"""
	def __init__(self, sizeX, sizeY, fullscreen = False):
		...
		"""
			We also create a variable to store the current frame we are on.
		"""
		self.frameCount = 0
		
		"""
			Create a placeholder shader. We use a lambda here since we merely
			return the character given.
		"""
		def basicShader( x, y, sizeX, sizeY, char): return char
		self.currentShader = basicShader
		
		"""
			If we are fullscreen, we resize the rendering surface
			at the outset.
		"""
		if fullscreen:
			ASCIIGLConstants.fullScreen = true
	...
	"""
		Applies a shader.
		YOU BEST HAD READ ABOUT HOW TO DO THIS.
	"""
	def setShader(self, shaderFunction):
		self.currentShader = shaderFunction
		
	"""
		Applies shader usage settings.
		
		VALUES from ASCIIGLConstants
		DISABLE_SHADING: 		Disables the use of shaders during 
								rendering.
		ENABLE_CHAR:	 		Enables the use of shading when drawing 
								non-textured elements.
		ENABLE_TEX:		 		Enables the use of shading when drawing 
								textured elements.
		ENABLE_CHAR_AND_TEX:	Enables the use of shading when drawing 
								any element.
		default = DISABLE_SHADING
	"""
	def setShaderUsage(self, shaderUsageMode):
		ASCIIGLConstants.shaderUsage = shaderUsageMode
	"""
	...
	"""
		Trims the framebuffer to the specified size, discarding
		all other stuff. This preserves all data currently in
		the framebuffer.
	"""
	def trimRenderingSurfaceToSize(newX, newY):
		#Create a list that is new new framebuffer for the rendering surface.
		newFrameBuffer = []
		
		#Go through and copy all of the data from the framebuffer.
		for i in range(newY):
			for k in range(newX):
				#If the new size is larger, we will inevitably start pulling 
				#indices that don't exist. If we do, we need to just put a 
				#clear-character there.
				try:
					#So we don't put part of the next line of the current framebuffer
					#into the current line of the new one, we must make sure
					#that if the current X sampling location is larger than the width
					#of the current framebuffer we just add a clear-character.
					if(k < self.sizeX):
						newFrameBuffer.append(self.frameBuffer[(i*self.sizeX)+k])
					else:
						newFrameBuffer.append(ASCIIGLConstants.clearCharacter)
				except IndexError:
					newFrameBuffer.append(ASCIIGLConstants.clearCharacter)
					
		#We now update the size of the surface so that culling and other
		#size-dependent operations still work.
		self.sizeX = newX
		self.sizeY = newY
		
		#Finally we update the reference to the framebuffer list to point
		#to our new one.
	...
	def checkWindowSize(self):
		if os.name == 'nt':
			...
			if(ASCIIGLConstants.fullScreen):
				if rows != self.height or columns != self.width:
					self.trimRenderingSurfaceToSize(rows, columns)
			else:
				if rows < self.height or columns < self.width:
					self.trimRenderingSurfaceToSize(rows, columns)
	...
	def character(self, x, y, character):
		...
		if(ASCIIGLConstants.colorMode == ASCIIGLConstants.EDIT_COLOR):
			...
			if(ASCIIGLConstants.shaderUsage == ASCIIGLConstants.ENABLE_CHAR or ASCIIGLConstants.shaderUsage == ASCIIGLConstants.ENABLE_CHAR_AND_TEX):
				self.frameBuffer[int(index)] = self.currentShader(x, y, self.sizeX, self.sizeY, character)
	...
	def applyTexel(self, x, y, texel):
		...
		if ASCIIGLConstants.texBlendingMode == ASCIIGLConstants.TEX_MULTIPLY:
			...
			if(ASCIIGLConstants.shaderUsage == ASCIIGLConstants.ENABLE_TEX or ASCIIGLConstants.shaderUsage == ASCIIGLConstants.ENABLE_CHAR_AND_TEX):
				self.frameBuffer[int(index)] = currentShader(x, y, self.sizeX, self.sizeY, character)
		else:
			self.frameBuffer[int(index% (self.sizeY*self.sizeX-1))] = texel
			if(ASCIIGLConstants.shaderUsage == ASCIIGLConstants.ENABLE_TEX or ASCIIGLConstants.shaderUsage == ASCIIGLConstants.ENABLE_CHAR_AND_TEX):
				self.frameBuffer[int(index)] = currentShader(x, y, self.sizeX, self.sizeY, character)
	...
	def blitToScreen(self, reallyClear):
		...
		if(ASCIIGLConstants.windowSizeCheck == ASCIIGLConstants.SIZE_CHECK_AT_INTERVAL and self.frameCount % ASCIIGLConstants.SIZE_CHECK_INTERVAL == 0):
			self.checkWindowSize()
		
		#Return the cursor to the origin of the coordinate system.
		print('\x1b[1;1H')
		
		#Reset the swap string.
		self.swapString = ""
		
		#Copy the framebuffer into the swap string.
